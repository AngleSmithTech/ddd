...
#if defined(NDEBUG)
# error "Digitalmoolah cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1344ce709ee10946f4272a378a1c4c40e815fff883ebe8fc5c9b1ca888ba4495");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Digitalmoolah: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Digitalmoolah Signed Message:\n";
...
{
    // Digitalmoolah: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Digitalmoolah: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Digitalmoolah: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Digitalmoolah: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Digitalmoolah: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Digitalmoolah: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6c749f42b0528beaeb2288638b18596778bafbb269418e075f37c00f69471881");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e81f189a63db7ebe94e3014e080d6ccd71dd13668231a989013924592a65a2e514ea83209ad64188622df7cf3b2be61f5fb29c02f3052bfde5d7cc938daa810a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1629320145;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085475671;

        if (fTestNet)
        {
            block.nTime    = 1629320189;
            block.nNonce   = 385498422;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x2435762edea76efce5e111a0fe369ea6cadb0167e4cd1830b48afec275299f9e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc9, 0xb9, 0xd1 }; // Digitalmoolah: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DigitalmoolahMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DigitalmoolahMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DigitalmoolahMiner(CWallet *pwallet)
{
    printf("DigitalmoolahMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("digitalmoolah-miner");
    ...
    try { loop {
        ...
        printf("Running DigitalmoolahMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DigitalmoolahMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DigitalmoolahMiner, pwallet));
}
